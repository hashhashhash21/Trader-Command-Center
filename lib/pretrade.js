...
function tradePlan({decision,price,rows}){price=num(price);const a=atr(rows),levels=swingLevels(rows);if(!price||!a||a<=0||!levels||!['LONG BIAS','SHORT BIAS'].includes(decision))return null;
 ...
 if(decision==='LONG BIAS'){const structural=levels.support<price&&price-levels.support<=2.5*a?levels.support-.18*a:null,invalidation=structural??price-1.05*a,risk=price-invalidation;if(risk<=0||risk>3*a)return null;const entryLow=Math.max(structural!=null?levels.support:price-.28*a,price-.35*a),entryHigh=price+.08*a,t1=Math.max(levels.resistance>price?levels.resistance:price,price+1.35*risk),t2=price+2.05*risk;if(entryLow>=entryHigh||t1<=price)return null;return{referencePrice:price,entryZone:[entryLow,entryHigh],invalidation,targets:[t1,t2],support:levels.support,resistance:levels.resistance,riskReward:(t2-price)/risk,atr:a,volatilityPct}}
 const structural=levels.resistance>price&&levels.resistance-price<=2.5*a?levels.resistance+.18*a:null,invalidation=structural??price+1.05*a,risk=invalidation-price;if(risk<=0||risk>3*a)return null;const entryLow=price-.08*a,entryHigh=Math.min(structural!=null?levels.resistance:price+.28*a,price+.35*a),t1=Math.min(levels.support<price?levels.support:price,price-1.35*risk),t2=price-2.05*risk;if(entryLow>=entryHigh||t1>=price)return null;return{referencePrice:price,entryZone:[entryLow,entryHigh],invalidation,targets:[t1,t2],support:levels.support,resistance:levels.resistance,riskReward:(price-t2)/risk,atr:a,volatilityPct}
}
